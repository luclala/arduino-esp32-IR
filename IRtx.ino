...
#define RMT_RX_GPIO_NUM   GPIO_NUM_16     /*!< GPIO number for receiver */
...
static void rmt_example_nec_tx_task() //void *pvParameters
{
    ...
    for(;;) {
        ...
        while(1) {
            ...
            i = nec_build_items(RMT_TX_CHANNEL, item + offset, item_num - offset, ((~addr) << 8) | addr, cmd);
            if(i < 0) {
                break;
            }
            cmd++;
...
void loop()
{
    ...
    //xTaskCreate(&rmt_example_nec_tx_task, "rmt_nec_tx_task", 2048, NULL, 5, NULL);
